...
# Define dilated CNN model
def define_dilated_model(patch_size, kernel_size):
    ...
    model.add(keras.layers.Conv2D(filters=128, padding='same',
                                  kernel_size=kernel_size,  dilation_rate=2))
    model.add(keras.layers.BatchNormalization())
    model.add(keras.layers.Activation('relu'))
...
def train_dilated_cnn(all_data, patch_size, kernel_size, save_dir, is_plot=False):

    # Load data
    (x_train, y_train), (x_valid, y_valid), (x_test, y_test), scaler, pca = pre_process_cnn_data(patch_size=patch_size, all_data=all_data)

    # Define dilated cnn model
    model = define_dilated_model(patch_size, kernel_size)
